...
- [x] Door types: archway (no block), wooden, locked / key, one `DoorDef` table in JSON (agent 3) -- the `DoorDef` table is `game/doors/*.json` (wood, wood_fold, iron, iron_sink, dungeon; `none` = archway); locked / key is not built (needs items) -- archway = door type `none`, in place. Locked doors and keys are a later item (need items), not part of this one
...
- [x] Enemy pathing: closed unlocked doors passable with small cost, enemies open them on contact (host only); locked doors count as walls, flow field rebuilt on lock changes; RoomGraph edges avoid locked doors (agent 3) -- enemy JSON `doors: none | open | phase` and the door-open predicate work, and a door change clears the FlowField; there are no locked doors, so avoiding them in `RoomGraph` is not needed yet -- confirmed by Orea 2026-09-24: enemies open, walk through and face through doors, and the ones that cannot interact with doors are blocked. Works as designed
- [x] DungeonMaker: two-click endpoint tool (click A, click B, snaps to the clicked edge), run bars, flip transforms connectors, undo stores the whole connector, free checkbox and door dropdown for the selected connector, list rows show run / width / free / door -- built 2026-09-24, not run in Godot. Left out: drag ends and hover (re-click a run to delete, draw again)
