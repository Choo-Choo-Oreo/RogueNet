...
- ~~Do wide openings (3+) ever get doors, or is that always an open arch?~~ Decided 2026-09-23: widths 1-2 get swinging wood or iron gates; widths 3-5 get a portcullis only (iron rise-and-fade or iron sink). Art for all of it is in `resources/gfx/doors/`: `Wood_W1`, `Wood_W2` (one atlas per width, the 2-high door is its own render) and `Iron_W1-5`, `IronSink_W1-5` (one atlas for the whole range, the pieces repeat), each as `.png` + `.json` manifest (piece rows, selection rule, 16x16 tile split) + `_Normal.png`.
