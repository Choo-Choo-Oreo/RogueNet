...
how far they can see a lit tile, `touch` the tiles they feel even in the dark. `hearing` is not
read yet; `smell` and `taste` are off. Light itself comes only from a held torch (the item's
