...
## Returns true if something was hit. `cause` is the action id (the hit and hurt sounds).
static func apply(caster: Node2D, tile: Vector2i, amount: int, type: String, cause: String = "") -> bool:
	...
	for creature: Node2D in caster.get_tree().get_nodes_in_group(target_team(caster)):
		...
		if tile.x < origin.x or tile.x >= origin.x + size or tile.y < origin.y or tile.y >= origin.y + size:
			continue
		_damage(creature, amount, type, cause)
		hit = true
	return hit
...
static func _damage(creature: Node2D, amount: int, type: String, cause: String) -> void:
	if creature.is_in_group("antagonist"):
		NetworkSync.report_minion_hit(int(str(creature.name)), amount, type, cause)
	else:
		NetworkSync.relay_player_hit(int(str(creature.name)), amount, type, cause)
