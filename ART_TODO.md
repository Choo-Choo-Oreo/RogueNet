...
- [~] Item icons, 16x16. The item list exists now (`game/items/`, the 40 gear pieces). Until an item has its own icon, its inventory slot shows the worn front view cropped to its pixels. That reads fine for helms, chests, legs, held items, amulets and back items, but worn gloves are one pixel per hand and boots two, so they show as dots. Most needed: an icon for each of the 4 gloves and 4 boots (`HeavyIronGauntlets`, `ArcaneGloves`, `ClericGloves`, `NecromancerGloves`, `HeavyIronSabatons`, `ArcaneSlippers`, `ClericBoots`, `NecromancerWraps`). To use one, add `"icon": "res://..."` to the item's JSON (suggested folder: `resources/gfx/ui/icons/items/`)
