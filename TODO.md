...
- [✓] Ghost player / soft-lock when the host starts a mission with a player not in it: `PlayerSpawner` spawned every connected peer. Now `NetworkSync.dive_members` (set in `_start_mission`, cleared on End Mission and `reset_session`) decides who spawns; players in town stay in town (untested; the same change covers the audit's ghost-avatar item)
...
- [✓] Other players' lights (2026-09-21, untested): other players show as a glow, not as shared vision. `LightMap._update_halos` puts a `light_halo.gdshader` sprite on each other player: same radius (`light_radius`) and brightness steps as the local light (shared `light_steps.gdshaderinc`), warm white (`halo_color`, `halo_strength` exports), added on top of the darkness so it lights nothing but itself. No flood per player (no extra cost, no player limit). Earlier attempt (a flood and shared vision per player) was removed. Known: the halo shines through walls
