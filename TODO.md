...
- [✓] Other players' lights: `LightMap.gd` keeps one `Light` (window, flood, texture) per player; `light_smooth.gdshader` takes the brightest of local + up to 3 others (`other_map_1..3`), `normal_lit.gdshader` shades from the nearest light (`other_lights`). Written 2026-09-21, not yet tested in game; limit of 4 players lit (local + 3), and each extra light costs about one more flood + paint per cell moved
