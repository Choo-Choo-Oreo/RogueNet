...
- [✓] Other players' lights, shared vision (decided 2026-09-21: a ranger can use a frontliner's eyes to see farther; untested): `LightMap.gd` keeps one `Light` (window, flood, texture) per player, `light_smooth.gdshader` takes the brightest of local + up to 3 others (`other_map_1..3`), `normal_lit.gdshader` shades from the strongest light (`other_lights`; nearest-light made a hard seam, adding doubled the flesh sparkle). Limit of 4 players lit, each extra light costs about one flood + paint per cell moved. Halo (glow-only) version was tried and dropped; `light_halo.gdshader` is unused and can be deleted; `light_steps.gdshaderinc` is still used by `light_smooth`
