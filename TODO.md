...
- [✗] Enemies beyond stats/roaming for the rat: 2026-09-22, `PlaceholderMouse.gd`/`.tscn` (the old stand-in for Dungeon Maker's enemy spawners) were deleted and their wander logic ported into a new generic `EnemyController.gd` (shares `GridMover`/`DirectionalAnimator`/`EntityStats` with `PlayerController.gd`, keyed by an `ENEMY_TYPES` dict like the player's knight/dwarf skin-swap, `size_tiles`-driven collision). Stats live at `game/entities/entities.enemies/rat.json`. Still needed: `EnemyController.tscn` isn't built yet (blocked on `rat_sprite_frames.tres` having lowercase animation names and no `Back` animation, mismatched with `DirectionalAnimator.gd`); Dungeon Maker's spawner placement UI was already disconnected from the real dungeon runtime before this (`DungeonAssembler.gd`/`DungeonPainter.gd` never read spawner data), so nothing spawns a rat in an actual run yet; only rat exists, no skeleton data
...
- [✓] `DungeonDebugView` no longer errors when `debug_dungeon_layout` is missing from a machine's `project.godot` (the friend's copy lacked it; check what else differs between your two `project.godot` files)
